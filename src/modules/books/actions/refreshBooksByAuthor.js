...
export const refreshBooksByAuthor = (author: Author) => {
  return (dispatch: Function, getState: Function) => {
    ...
    const booksToIgnore = books.filter((b: Book) => b.author.id !== author.id)
    const booksToUpdate = books.filter((b: Book) => b.author.id === author.id)

    booksToUpdate.forEach((book: Book) => refreshBookAuthor(book, [author]))

    dispatch(fetchBooks([...booksToIgnore, ...booksToUpdate]))
  }
}
